...
$sql = "SELECT team_name, region, ranking, founded_year, coach_id FROM team";
...
<title>Teams List — Esports Tournament Management System</title>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pt-eyebrow::before {
    ...
    background: var(--gold); box-shadow: var(--glow-g);
  }
  ...
  h1 span {
    background: linear-gradient(90deg, var(--text) 30%, var(--gold));
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
    background-clip: text;
  }
  ...
  td:nth-child(3) { text-align: center; width: 100px; }

  /* founded year col */
  thead th:nth-child(4),
  td:nth-child(4) { text-align: center; width: 130px; }
  ...
  .team-icon { font-size: 15px; transition: transform 0.25s; }
  tbody tr:hover .team-icon { transform: scale(1.2); }
  ...
  tbody tr:hover .region-dot { opacity: 1; box-shadow: 0 0 6px var(--cyan); }
  ...
  .rank-n  { color: var(--gold); }
  tbody tr:hover .rank-n { text-shadow: 0 0 10px rgba(240,192,64,0.4); }
  ...
  tbody tr:hover .year-val { color: var(--text); }
  ...
  .tf-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--gold); }
  .tf-query {
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 10px; letter-spacing: 2px; text-transform: uppercase; color: var(--muted);
  }
  .tf-query span { color: var(--gold); font-style: italic; letter-spacing: 1px; }
...
<nav>
  ...
  <a href="teams_menu.html" class="active"><span>🛡️</span>Teams</a>
...
<div class="breadcrumb">
  ...
  <a href="teams_menu.html">Teams</a>
  <span class="breadcrumb-sep">›</span>
  <span class="breadcrumb-cur">Teams List</span>
</div>
...
<main>
  ...
  <div class="page-top">
    <!-- <div class="pt-eyebrow">Teams — SELECT</div> -->
    <h1><span>Teams List</span></h1>
  </div>
  ...
  <div class="table-panel">
    <div class="table-scroll">
      <table>
        ...
        <tbody>
          <?php if ($count === 0): ?>
            <tr class="empty-row"><td colspan="6">No teams found</td></tr>
          <?php else: ?>
            <?php foreach ($rows as $i => $row):
              $rank = intval($row['ranking']);
              $rank_class = match(true) {
                $rank === 1 => 'rank-1',
                $rank === 2 => 'rank-2',
                $rank === 3 => 'rank-3',
                default     => 'rank-n',
            ...
            <tr>
              ...
              <td>
                <span class="rank-badge <?= $rank_class ?>">
                  <?= $rank === 1 ? '👑' : '#' . htmlspecialchars($row['ranking']) ?>
                </span>
              </td>
              <td><span class="year-val"><?= htmlspecialchars($row['founded_year']) ?></span></td>
              <td><span class="coach-pill"><?= htmlspecialchars($row['coach_id']) ?></span></td>
              <td>
    ...
    <div class="table-foot">
      ...
      <!-- <div class="tf-query">Query: <span>SELECT team_name, region, ranking, founded_year, coach_id FROM team</span></div> -->
    </div>
  </div>
...
<footer>
  ...
  <a href="teams_menu.html" class="footer-back">← Back to Teams</a>
</footer>
