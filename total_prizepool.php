...
$sql = "SELECT SUM(total_amount) AS total_prize_money FROM prizepool";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_assoc($result);
$total = $row['total_prize_money'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Total Prize Pool — Esports Tournament Management System</title>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ring-2 { width: 360px; height: 360px; border-color: rgba(240,192,64,0.03); }
  .ring-3 { width: 460px; height: 460px; border-color: rgba(240,192,64,0.015); }
  ...
  @keyframes amountPulse {
    0%,100% {
      text-shadow: 0 0 20px rgba(240,192,64,0.4), 0 0 60px rgba(240,192,64,0.1);
    }
    50% {
      text-shadow: 0 0 50px rgba(240,192,64,0.7), 0 0 100px rgba(240,192,64,0.25);
    }
  }
  ...
  /* null/zero state */
  .prize-amount.empty {
    color: var(--muted); font-size: clamp(24px, 4vw, 40px);
    text-shadow: none; animation: none; letter-spacing: -1px;
  }
  ...
  .qb-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--gold); }
  .qb-text { color: var(--gold); font-style: italic; }
  ...
  @media (max-width: 700px) {
    ...
    .prize-card { padding: 48px 32px 44px; }
  }
</style>
</head>
<body>
...
<div class="breadcrumb">
  ...
  <span class="breadcrumb-sep">›</span>
  <span class="breadcrumb-cur">Total Prize Pool</span>
</div>

<!-- MAIN -->
<main>

  <!-- <div class="page-eyebrow">Analytics — SUM Aggregate</div> -->
  ...
  <div class="prize-card">
    ...
    <div class="card-title">Total Prize Pool of All Tournaments</div>

    <span class="trophy-icon">🏆</span>

    <?php if($total !== null && floatval($total) > 0): ?>
      <div class="prize-amount">
        <span class="currency">₹</span><?= number_format(floatval($total), 2) ?>
      </div>
    <?php else: ?>
      <div class="prize-amount empty">No prize data available</div>
    <?php endif; ?>
    ...
    <!-- <div class="query-badge">
      ...
      <span>Query:</span>
      <span class="qb-text">SELECT SUM(total_amount) FROM prizepool</span>
    </div>
  </div> -->
