...
$sql = "SELECT team.team_name, COUNT(player.player_id) AS total_players
FROM team
JOIN player ON team.team_id = player.team_id
GROUP BY team.team_name";
$result = mysqli_query($conn,$sql);

/* collect rows for max value (bar scaling) */
$rows = [];
if ($result) {
...
$count    = count($rows);
$max_size = $count > 0 ? max(array_column($rows, 'total_players')) : 1;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Players Per Team — Esports Tournament Management System</title>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  td:last-child      { text-align: right; width: 160px; }
  ...
  td { padding: 0 24px; height: 64px; font-size: 14px; font-weight: 500; color: var(--text); white-space: nowrap; }
  ...
  tbody tr:hover .team-shield { transform: scale(1.2); }
  ...
  tbody tr:first-child .fill-bar { background: linear-gradient(90deg, var(--purple), var(--cyan)); }

  .count-num {
    font-family: 'Orbitron', monospace;
    font-size: 16px; font-weight: 900;
    color: var(--purple);
    text-shadow: 0 0 12px rgba(160,96,255,0.3);
    min-width: 28px; text-align: right;
  }
  tbody tr:hover .count-num { color: var(--cyan); text-shadow: 0 0 12px rgba(92,207,255,0.4); }

  /* empty */
  .empty-row td {
    text-align: center; padding: 56px 24px; height: auto;
    font-family: 'Barlow Condensed', sans-serif;
...
<div class="breadcrumb">
  ...
  <span class="breadcrumb-sep">›</span>
  <span class="breadcrumb-cur">Players Per Team</span>
</div>
...
<main>
  ...
  <div class="page-top">
    <!-- <div class="pt-eyebrow">Teams — COUNT + GROUP BY</div> -->
    <h1><span>Players in Each Team</span></h1>
  </div>
  ...
  <div class="table-panel">
        ...
        <tbody>
          ...
          <?php else: ?>
            <?php foreach ($rows as $i => $row):
              $total   = intval($row['total_players']);
              $bar_pct = $max_size > 0 ? round(($total / $max_size) * 100) : 0;
            ?>
            <tr>
              ...
              <td>
                <div class="td-count">
                  <div class="fill-bar-track">
                    <div class="fill-bar" style="width:<?= $bar_pct ?>%"></div>
                  </div>
    ...
    <div class="table-foot">
      <div class="tf-label">
        ...
        <?= $count ?> team<?= $count !== 1 ? 's' : '' ?> listed
      </div>
      <!-- <div class="tf-query">Query: <span>COUNT(player_id) GROUP BY team_name</span></div> -->
    </div>
  </div>
