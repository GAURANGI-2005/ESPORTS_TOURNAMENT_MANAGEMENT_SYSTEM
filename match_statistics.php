...
$sql = "SELECT player.player_name, matchstats.match_id,
matchstats.kills, matchstats.assists, matchstats.damage_done
FROM player
...
$count       = count($rows);
$max_kills   = $count > 0 ? max(array_column($rows, 'kills'))       : 1;
$max_damage  = $count > 0 ? max(array_column($rows, 'damage_done')) : 1;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Match Statistics — Esports Tournament Management System</title>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  td:nth-child(2) { text-align: center; width: 110px; }
  td:nth-child(3) { text-align: center; width: 110px; }
  td:nth-child(4) { text-align: center; width: 110px; }
  td:last-child   { text-align: right; min-width: 180px; }
  ...
  tbody tr:hover .kills-val { text-shadow: 0 0 16px rgba(255,68,102,0.5); }
  ...
  tbody tr:hover .dmg-val { color: var(--gold); }
  ...
  .tf-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--red); }
  .tf-query {
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 10px; letter-spacing: 2px; text-transform: uppercase; color: var(--muted);
  }
  .tf-query span { color: var(--red); font-style: italic; letter-spacing: 1px; }
...
<div class="breadcrumb">
  ...
  <span class="breadcrumb-sep">›</span>
  <span class="breadcrumb-cur">Match Statistics</span>
</div>
...
<main>
  ...
  <div class="page-top">
    <!-- <div class="pt-eyebrow">Analytics — player JOIN matchstats</div> -->
    <h1><span>Match Statistics</span></h1>
  </div>
    ...
    <div class="table-scroll">
      <table>
        <thead>
          <tr>
            <th>Player</th>
            <th class="th-center">Match ID</th>
            <th class="th-center">Kills</th>
            <th class="th-center">Assists</th>
            <th class="th-right">Damage Done</th>
          </tr>
        </thead>
        <tbody>
          <?php if ($count === 0): ?>
            <tr class="empty-row"><td colspan="5">No match data found</td></tr>
          <?php else: ?>
            <?php foreach ($rows as $row):
              $kills   = intval($row['kills']);
              $damage  = intval($row['damage_done']);
              $dmg_pct = $max_damage > 0 ? round(($damage / $max_damage) * 100) : 0;
            ?>
            <tr>
              <td class="td-player"><?= htmlspecialchars($row['player_name']) ?></td>
              <td><span class="match-pill">#<?= htmlspecialchars($row['match_id']) ?></span></td>
              <td><span class="kills-val"><?= $kills ?></span></td>
              <td><span class="assists-val"><?= htmlspecialchars($row['assists']) ?></span></td>
              <td>
                <div class="dmg-cell">
                  <div class="dmg-bar-track">
                    <div class="dmg-bar" style="width:<?= $dmg_pct ?>%"></div>
                  </div>
                  <span class="dmg-val"><?= number_format($damage) ?></span>
                </div>
              </td>
            </tr>
            <?php endforeach; ?>
